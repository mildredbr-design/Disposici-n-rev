...
def simulador(capital, tin, cuota_mensual, fecha_inicio,
    ...
    if fecha_joker is not None:
        # Recibo siguiente al de la fecha_joker
        ...
        # Avanzar hasta encontrar el recibo cuyo periodo contiene fecha_joker
        ...
        # recibo_candidato es el primer recibo posterior a fecha_joker
        # Aplicar regla de bloqueo
        fb_candidato = fecha_bloqueo_para_mes(recibo_candidato)
        if fecha_joker < fb_candidato:
            fecha_joker_recibo = recibo_candidato
        else:
